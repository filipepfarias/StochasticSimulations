...
# Ref https://doi.org/10.1063/1.4802990
...
function langevin_baoab(potential, max_time, dt, gamma, kBT, initial_position, initial_velocity,save_frequency=3)
    ...
    t = 0
    step_number = 0
    positions = Matrix{Float64}(undef,0,2)
    velocities = Matrix{Float64}(undef,0,2)
    ...
    while t<max_time
        ...
        if ((step_number%save_frequency == 0) || (step_number>0))
            ...
            positions = vcat(positions,x)
            velocities = vcat(velocities,v)
